...
export default function Footer({ locale = "en" }: Props) {
  ...
  const POWERED_BY_NAME = process.env.NEXT_PUBLIC_POWERED_BY_NAME || "Nuremberg Group";

  const year = 2026;

  return (
    <footer dir={isArabic ? "rtl" : "ltr"} className="w-full bg-white text-slate-700 pt-10 px-6 md:px-16 lg:px-24 xl:px-32">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-14">
        <div className="sm:col-span-2 lg:col-span-1">
          <Link href={ABOUT_ROUTE} className="inline-flex items-center justify-center">
        ...
        <div className="flex flex-col lg:items-start lg:justify-start lg:pt-11">
          <div className="flex flex-col text-lg space-y-3">
            <h2 className="font-semibold mb-3 text-gray-800">{isArabic ? "البرامج" : "Programs & About"}</h2>
            <Link href={LIFE_ROUTE} className="hover:text-slate-600 transition text-lg">
              {isArabic ? "برنامج الحياة" : "Life program"}
            </Link>
            <Link href={PROFESSIONAL_ROUTE} className="hover:text-slate-600 transition text-lg">
              {isArabic ? "البرامج المهنية" : "Professional programs"}
            </Link>
            <Link href={ABOUT_ROUTE} className="hover:text-slate-600 transition text-lg">
              {isArabic ? "من نحن" : "About"}
            </Link>
            <Link href={MISSION_ROUTE} className="hover:text-slate-600 transition text-lg">
...
>
  ...
  <div className="text-lg space-y-4 max-w-sm flex flex-col items-start">
    ...
    <div className=" flex items-center gap-4 rtl:flex-row-reverse">
      <Link href={SOCIAL_FB} target="_blank" className="hover:text-slate-600 transition" aria-label="Facebook">
        <SiFacebook size={32} />
      </Link>

      <Link href={SOCIAL_IG} target="_blank" className="hover:text-slate-600 transition" aria-label="Instagram">
        <SiInstagram size={32} />
      </Link>
      <Link href={SOCIAL_X} target="_blank" className="hover:text-slate-600 transition" aria-label="X">
        <SiX size={32} />
      </Link>

      <Link href={SOCIAL_YT} target="_blank" className="hover:text-slate-600 transition" aria-label="YouTube">
        <SiYoutube size={32} />
      </Link>
    </div>
  </div>
     ...
     <div className="pt-6 mt-8 border-t mb-4 border-slate-200 flex items-center justify-center text-center">
  <p className="text-base flex flex-row">
    {isArabic ? (
      <>
        <Copyright width={16} height={16} className="m-1"/> {year} Etqansa. جميع الحقوق محفوظة •{" "}
         مدعوم من 
        <a
          href={NUREMBERG_URL}
          target="_blank"
      ...
      <>
        <Copyright width={16} height={16} className="m-1"/> {year} Etqansa. All rights reserved . Powered by{" "}
        <a
          href={NUREMBERG_URL}
          target="_blank"
          rel="noopener noreferrer"
